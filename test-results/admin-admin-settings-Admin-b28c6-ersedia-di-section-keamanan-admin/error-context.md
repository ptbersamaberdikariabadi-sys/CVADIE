...
- Name: admin\admin-settings.spec.ts >> Admin Pengaturan — /admin/settings >> field email login (disabled) tersedia di section keamanan
- Location: tests\e2e\admin\admin-settings.spec.ts:47:7
...
Locator: locator('input[type="email"][disabled]').first()
Expected: visible
Timeout: 5000ms
...
Call log:
  ...
  - waiting for locator('input[type="email"][disabled]').first()
