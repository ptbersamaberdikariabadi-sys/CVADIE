...
- Name: admin\admin-products.spec.ts >> Admin Form Tambah Produk — /admin/products/new >> form produk dengan field-field utama tampil
- Location: tests\e2e\admin\admin-products.spec.ts:97:7
...
Locator: locator('input[name="name"], input[placeholder*="Nama"], input[id*="name"]').first().or(locator('label:has-text("Nama")').first()).first()
Expected: visible
Timeout: 10000ms
...
Call log:
  ...
  - waiting for locator('input[name="name"], input[placeholder*="Nama"], input[id*="name"]').first().or(locator('label:has-text("Nama")').first()).first()
