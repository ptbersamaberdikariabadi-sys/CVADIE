...
- Name: admin\admin-products.spec.ts >> Admin Katalog Produk — /admin/products >> tabel produk menampilkan data atau pesan kosong
- Location: tests\e2e\admin\admin-products.spec.ts:47:7
...
Locator: locator('tbody tr').first().or(locator('text=/Belum ada produk/i').first()).first()
Expected: visible
Timeout: 10000ms
...
Call log:
  ...
  - waiting for locator('tbody tr').first().or(locator('text=/Belum ada produk/i').first()).first()
