...
- Name: admin\admin-rfq.spec.ts >> Admin RFQ — /admin/rfq >> tabel menampilkan data atau pesan kosong
- Location: tests\e2e\admin\admin-rfq.spec.ts:32:7
...
Locator: locator('tbody tr').first().or(locator('text="Belum ada prospek yang masuk."')).first()
Expected: visible
Timeout: 10000ms
...
Call log:
  ...
  - waiting for locator('tbody tr').first().or(locator('text="Belum ada prospek yang masuk."')).first()
