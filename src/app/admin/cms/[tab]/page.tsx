...
export default async function AdminCmsPage({ params }: { params: Promise<{ tab: string }> }) {
  ...
  // Define tab map for display
  const tabTitles: Record<string, string> = {
    'hero_section': 'Seksi Hero',
    'why_choose_us': 'Seksi Alasan',
    'trust_grid': 'Pilar Kepercayaan',
    'services': 'Kategori Layanan',
    'workflow': 'Seksi Alur Kerja',
    'cta_banner': 'Banner CTA Bawah',
    'about_page': 'Halaman Tentang Kami'
  };

  const title = tabTitles[tab] || 'Kustomisasi CMS';

  return (
    <div className="space-y-6 max-w-5xl">
      <div>
        <h2 className="text-xl font-bold text-gray-900">{title}</h2>
        <p className="text-sm text-gray-500 mt-1">Ubah teks, gambar, dan bagian-bagian di halaman publik secara langsung.</p>
      </div>

      <CmsEditor initialContent={initialContent} activeTabProp={tab} />
    </div>
  );
}
